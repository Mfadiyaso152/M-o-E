...
export function CompletePhoneModal({ user, onSavePhone }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm touch-none" dir="rtl">
      <motion.div
        initial={{ scale: 0.94, opacity: 0, y: 15 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        transition={{ duration: 0.25, ease: 'easeOut' }}
        className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-[#E8E2D8] text-[#192A1D] space-y-4 touch-auto"
        onClick={e => e.stopPropagation()}
      >
        <div className="text-center space-y-2">
          <div className="w-12 h-12 bg-[#1C3022] text-[#C5B198] rounded-2xl flex items-center justify-center mx-auto shadow-md">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <ArrowRight className="w-4 h-4 rotate-180 text-[#C5B198]" />
              </>
            )}
          </button>
        </form>
